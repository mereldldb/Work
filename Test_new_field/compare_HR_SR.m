...
lambda_t = 1e-15;%1e-15;%6e-16;%2e-16;
...
for l = 1:9
      [y{l},err] = admm_tv(N, b{l}, sparse(npx*npy/L^2,1), speye(npx^2/L^2),speye(length(b{l})), R, F, iter_admm, tol_admm, iter_CG, tol_CG, lambda, a_admm, y_mod{l});
...
%     lambda_ep = str2num(char(params(2)));
    ...
    x_sr_tv = admm_tv(A, bb, sparse(npx*npy,1), speye(length(x_mod)),speye(length(bb)), R, F, iter_admm, tol_admm, iter_CG, tol_CG, lambda_tv, 10*lambda_tv, x_mod);
 
    error2_sr_tv = norm(max(0,real(x_sr_tv))-x_mod,2);
    error1_sr_tv = norm(max(0,real(x_sr_tv))-x_mod,1);
%     [error2_sr_tv, error1_sr_tv]
 
    figure(1)
    subplot(2,2,3)
    imagesc(reshape(real(x_sr_tv),npy,npx),[0 1]); colorbar
    axis square
